    ...
    for i in range(imputed_img.shape[1]):
        for j in range(imputed_img.shape[2]):
            ...
            if spot_coords in integral_coords:
                ...
                # Ensure the original spots retain their gene expression values
                imputed_counts.iloc[flag, :] = imputed_img[:, i, j]  # Original spots retain values
            else:
                ...
                # Here, fill with imputed gene expression values for new coordinates
...
def integrate_imputed_data(original_adata, imputed_adata):
    ...
    predicted_expression = pd.DataFrame(
        original_adata.X,
        index=original_adata.obs_names,
        columns=original_adata.var_names
    )
